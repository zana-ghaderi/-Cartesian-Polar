...
__host__  void initTestCases(void) {
	...
	for(i=0;i<gN;i++) {
		for(j=0;j<gN;j++) {                 
			...
			refSetRCART((void*)array0, i, j, gN, floatToSHORT16 (i/(j+1)));
			refSetICART((void*)array0, i, j, gN,-floatToSHORT16 (i/(j+1)));
...
//////////////////////////////////////////my code/////////////////
__global__ void ComputeCart2Pol(void *POL, void *CART, int N, float R, float theta,int WT,int *mem) {
...
////////////////////////////////////////////////
 __host__ void testCart2pol() {
	...
	for(testnum=5;testnum<6;testnum++) {
		...
		cudaBindTexture(0, tex, mem, 2*N*N*sizeof(int));
			...
			cudaMemcpy(CART_d, test[testnum].arrayy , CART_size, cudaMemcpyHostToDevice);
				...
				/*GPU computation */		
			ComputeCart2Pol<<< dimGrid,dimBlock>>>((void *)POL_d, (void *)CART_d, test[testnum].N, test[testnum].R, test[testnum].theta, WT,(int*)mem);
