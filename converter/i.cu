...
__host__  void initTestCases(void) {
	...
	for(i=0;i<gN;i++) {
		for(j=0;j<gN;j++) {                 
			...
			refSetRCART((void*)array1, i, j, gN, floatToSHORT16(pow(1.05,(i-j))));
			refSetICART((void*)array1, i, j, gN, -floatToSHORT16(pow(1.05,(i-j))));
		}
	}
...
//////////////////////////////////////////my code/////////////////
__global__ void ComputeCart2Pol(void *POL, void *CART, int N, float R, float theta,int WT,int *Ar_INDEX_d) {
	...
	int INDEX;
			...
			int *pr= (int *)Ar_INDEX_d +(t*N+r)*2;
			lli = *pr;
			int *pt= (int *)Ar_INDEX_d +(t*N+r)*2+1;
			llj = *pt;
...
void ComputeCart2PolOnCpu(void *POL, void *CART, int N, float R, float theta) {
 ...
 __host__ void testCart2pol() {
	...
	for(testnum=0;testnum < 12; testnum++) {
		...
		{	/* run and time your implementation */	
			...
			size_t CART_size = N*N * sizeof(unsigned int);
			size_t POL_size = N*N * sizeof(unsigned int);
			size_t Ar_INDEX_size = N*N * sizeof(int);
			//////////////////////////compute index for kernel and send it to kernel

			float dx,dy,x,y;

			float R = test[testnum].R;
			float theta = test[testnum].theta;

			int *Ar_INDEX = (int*)malloc(2*Ar_INDEX_size);
			...
			dy=((R+1)*(sin(theta)))/(N-1);
				...
				for(int t=0; t < N; t++){
					 for(int r=0; r < N; r++) {
						 ...
						 Ar_INDEX[2 * (r + t * N )] = lli;
						 Ar_INDEX[2 * (r + t * N ) + 1] = llj;
					}
				 }
			cudaMalloc((void **) &Ar_INDEX_d, 2*Ar_INDEX_size);
			cudaMemcpy(Ar_INDEX_d, Ar_INDEX, 2*Ar_INDEX_size, cudaMemcpyHostToDevice );
		

			//////////////////////////end of index computation for cuda
			...
			cudaMemcpy(CART_d, test[testnum].arrayy , CART_size, cudaMemcpyHostToDevice);
				/*GPU computation */		
			ComputeCart2Pol<<< dimGrid,dimBlock>>>((void *)POL_d, (void*)CART_d, test[testnum].N, test[testnum].R, test[testnum].theta, WT,(int*)Ar_INDEX_d);
			...
			{
				start = clock();
				ComputeCart2PolOnCpu((void *)result, (void *)test[testnum].arrayy, test[testnum].N, test[testnum].R, test[testnum].theta);
				end = clock();
				printf("%f\n",(double)(end-start)/(double)(CLOCKS_PER_SEC));
			}
		}
