...
void Cart2PolOnCpu(void *POL, void *CART, int N, float R, float theta) {
    int r, t;
    float dx, dy, x, y;
    int lli, llj;
...
void initTestCases(void) {
    ...
    test[0].array = (void *) array0;
    test[1].array = (void *) array1;
    test[2].array = (void *) array0;
    test[3].array = (void *) array1;
    test[4].array = (void *) array0;
    test[5].array = (void *) array1;
    test[6].array = (void *) array0;
    test[7].array = (void *) array1;
    test[8].array = (void *) array0;
    test[9].array = (void *) array1;
    test[10].array = (void *) array0;
    test[11].array = (void *) array1;
}
...
__global__ void ComputeCart2Pol (DCX *POL_d ,DCX *CART_d ,int N ,float R ,float Tetha , int WT) {
    ...
    if ((iy == N - 1) && (ix == N - 1)) {
        ...
        POL_d[ID_BLK_Rad * N + ID_BLK_Tetha].x =
                (CART_d[iy * N + ix].x + CART_d[iy * N + ix + 1].x + CART_d[(iy + 1) * N + ix].x +
                 CART_d[(iy + 1) * N + ix + 1].x) / 4;

        POL_d[ID_BLK_Rad * N + ID_BLK_Tetha].y =
                (CART_d[iy * N + ix].y + CART_d[iy * N + ix + 1].y + CART_d[(iy + 1) * N + ix].y +
                 CART_d[(iy + 1) * N + ix + 1].y) / 4;
    }
}
__global__ void ComputeCart2PolV2(void *POL, void *CART, int N, double R, double theta) {
    int r, t;
    double dx, dy, x, y;
    int lli, llj;
...
void convert(void * temp_pol,void * temp_cart,int N, float R,float Tetha) {
    int NB, WT;
    DCX *CART;
    DCX *POL;
...
void testCart2pol() {
    ...
    for (testnum = 0; testnum < 12; testnum++) {
               ...
               testnum, test[testnum].N, (int) (test[testnum].R * 1000000), (int) (test[testnum].theta * 1000000));
        {
            ...
            refComputeCart2Pol((void *) Refresult, (void *) test[testnum].array, test[testnum].N, test[testnum].R,
                               test[testnum].theta);
            end = clock();
        ...
        {    /* run and time your implementation */
            ...
            clock_t end;
            start = clock();
            convert((void *) result, (void *) test[testnum].array, test[testnum].N, test[testnum].R,
                     test[testnum].theta);
            end = clock();
            printf("%f \n", (end - start) / ((double) CLOCKS_PER_SEC));
        }
        ...
        {
            double dx = ((test[testnum].R + 1) - (test[testnum].R * (cos((double) test[testnum].theta)))) /
                        (test[testnum].N - 1);
            ...
            for (i = 1; i < (test[testnum].N - 1); i++) {
                for (j = 1; j < (test[testnum].N - 1); j++) {
                    double posx = (test[testnum].R + (double) i / (test[testnum].N - 1)) *
                                  cos(test[testnum].theta * (double) j / (test[testnum].N - 1));
                    double posy = (test[testnum].R + (double) i / (test[testnum].N - 1)) *
                                  sin(test[testnum].theta * (double) j / (test[testnum].N - 1));
                    int omit = (((int) (posy / dy) + 1) < (posy / dy + OFFSET)) ||
                               (((int) (posy / dy)) > (posy / dy - OFFSET)) ||
                               (((int) ((posx - test[testnum].R * cos(test[testnum].theta)) / dx) + 1) <
                                ((posx - test[testnum].R * cos(test[testnum].theta)) / dx + OFFSET)) ||
                               (((int) ((posx - test[testnum].R * cos(test[testnum].theta)) / dx)) >
                                ((posx - test[testnum].R * cos(test[testnum].theta)) / dx - OFFSET));
